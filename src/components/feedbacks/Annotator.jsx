...
const AnnotationInput = ({annotation}) => (
  <div>
    <input
      type="text"
      value={annotation}
      style={styles.inputAnnotation}
    />
  </div>
)


export default class AnnotatorComponent extends React.Component {
  constructor(props) {
    super(props)
    // ---
  ...
  clearCanvas() {
      ...
      this.setState({touchStack: [], artworkSrc: null, annotationStack: []})
  ...
  sketchpad_mouseUp() {
      this.mouseDown = 0
      // add annotation input field
      const annotationStack = this.state.annotationStack
      annotationStack.push("TODO")
      this.setState({annotationStack})
  }
  ...
  render() {
    ...
    const { touchStack = [], imgSrc, annotationStack } = this.state
        ...
        <Block style={styles.sketchpadapp}>
          <Row>
            ...
            <Col width="30">
              ...
              <h2>Annotations</h2>
              {
                annotationStack.map((annotation, index) => (
                  <AnnotationInput key={`a_${index}`} value={annotation} />
                ))
              }
